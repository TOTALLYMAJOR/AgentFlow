...
export async function reconcileInitiative(context: AgentFlowContext, id: string): Promise<Record<string, unknown>> {
  ...
  if (initiative.status === "partial" && !builds.some((build) => ["failed", "cancelled"].includes(build.status)) && builds.some((build) => ["ready", "running"].includes(build.status))) initiative = context.store.initiatives.transition(id, "running", "initiative.repository_retry_resumed");
